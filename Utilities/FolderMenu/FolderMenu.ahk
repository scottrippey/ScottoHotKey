...
;
;-BUG---------
; ! reload didnt clear hotkey
; * submenu loop
; * submenu name can't contain '&'
;
;-NEW---------
; * GUI
;   * manage fav
...
;
;-CHANGE------
; * read from ini when open fav
; * change labels to functions
;   * openfav
; v addfav in dialog
;   ? find a better way to get path
; * customize program
;   * for hotkey2
;   * for opensel
;     v use a menu *canceled*
;
;-DONE--------
; v open reg entry
; v fixd submenu bug (duplicate error)
; v shift-click works like ctrl-click
; v ctrl-click to create menu for subforders of that item
;   v sort
; v submenu
; v options
;   v notray
; v addfav gui
;   v check name and path
;   v hold shift to skip check
; v edit wait notepad close, readconfig *canceled*
; v addfav from cmd
; v add tool menu to fav menu
;   v move 'addfav' in
;     v hotkey for addfav
; v addfav GUI
;   v addfav from otherapp
; v hotkey for edit reload exit etc.
; v open selected path
; v get class and check edit1
;   ? auto write into config
; v use '-' as separator
; v check if the path vaild
; v addfav - when name duplicated
; v add support for more apps
; v read config fav section
;===============================================================================
...
Menu, Tray, Add
Menu, Tray, Add, &Add Favorite, f_NewFavorite
Menu, Tray, Add
Menu, Tray, Add, &Reload, f_TrayReload
Menu, Tray, Add, &Edit, f_TrayEdit
Menu, Tray, Add
Menu, Tray, Add, E&xit, f_TrayExit
Menu, Tray, Default, &Folder Menu
Menu, Tray, Click, 1

Menu, Tool, Add, &Add Favorite, f_NewFavorite
Menu, Tool, Add,
Menu, Tool, Add, &Reload, f_TrayReload
Menu, Tool, Add, &Edit, f_TrayEdit
Menu, Tool, Add
Menu, Tool, Add, E&xit, f_TrayExit
...
GetKeyState, f_OpenFavCState, Shift
if f_OpenFavCState = D
{
	if f_OpenTempMenu(f_OpenFavPath)
		return
...
;----- in filezilla ----;
else if w_Class in wxWindowClassNR	; In FileZilla, switch folders.
{
	if w_Edit1Pos <> 	; And it has an Edit5 control.
	{
		ControlClick, Edit5, ahk_id %w_WinID%
		ControlSetText, Edit5, %f_OpenFavPath%, ahk_id %w_WinID%
		; Tekl reported the following: "If I want to change to Folder L:\folder
		; then the addressbar shows http://www.L:\folder.com. To solve this,
		; I added a {right} before {Enter}":
		ControlSend, Edit5, {Right}{Enter}, ahk_id %w_WinID%
		return
	}
	; else fall through to the bottom of the subroutine to take standard action.
}
...
if w_Class in wxWindowClassNR	; Filezilla 3
	ControlGetPos, w_Edit1Pos,,,, Edit5, ahk_id %w_WinID%

;----- check if display the menu -----;
if w_Class in #32770,CabinetWClass,ExploreWClass,%f_OtherApps%,wxWindowClassNR
{
...
Gui, Add, Text, x10 y8  w40  h14, Name:
Gui, Add, Edit, x10 y23 w233 h21 vf_NewFavName, %f_NewFavName%
Gui, Add, Text, x10 y53 w40  h14, Path:
Gui, Add, Edit, x10 y68 w233 h21 vf_NewFavPath, %f_NewFavPath%
Gui, Add, Button, x16  y99 w100 h21 gf_NewFavBFolder, &Browse Folders...
Gui, Add, Button, x136 y99 w100 h21 gf_NewFavBFile, B&rowse Files...
Gui, Add, Button, x253 y23 w75 h21 gf_NewFavOK Default, &OK
Gui, Add, Button, x253 y49 w75 h21 gf_NewFavCancel, &Cancel
Gui, Font, cRed
Gui, Add, Text, x64 y8  w180 h14 vf_NewFavErr1, This favorite name already exist.
Gui, Add, Text, x64 y53 w180 h14 vf_NewFavErr2, This path did not exist.
Gui, Add, Text, x253 y79 w70 h40 vf_NewFavTip , Hold shift and click OK to skip the check.
GuiControl, Hide, f_NewFavErr1
GuiControl, Hide, f_NewFavErr2
GuiControl, Hide, f_NewFavTip
Gui, Show, h128w w343, Add Favorite
return
...
StringReplace, f_Selected, f_Selected, ¢@, \, All
StringRight, LastChar, f_Selected, 1
if LastChar = \	; Remove the trailing backslash.
	StringTrimRight, f_Selected, f_Selected, 1
...
RegRead, MyComputer, HKEY_LOCAL_MACHINE, SOFTWARE\Classes\CLSID\{20D04FE0-3AEA-1069-A2D8-08002B30309D}
Run, explore %f_Selected%, , UseErrorLevel
if ErrorLevel
	Run, %f_Selected%, , UseErrorLevel
	if ErrorLevel
	{
		StringLeft, f_SelectedFirst2, f_Selected, 2
		StringLeft, f_SelectedFirst4, f_Selected, 4
		if f_SelectedFirst2 = HK
		{
			if f_SelectedFirst4 = HKCR
				StringReplace, f_Selected, f_Selected, HKCR, HKEY_CLASSES_ROOT
			if f_SelectedFirst4 = HKCU
				StringReplace, f_Selected, f_Selected, HKCU, HKEY_CURRENT_USER 
			if f_SelectedFirst4 = HKLM
				StringReplace, f_Selected, f_Selected, HKLM, HKEY_LOCAL_MACHINE
		}
		StringLeft, f_SelectedFirst4, f_Selected, 4
		if f_SelectedFirst4 = HKEY
		{
			RegWrite, REG_SZ, HKEY_CURRENT_USER, Software\Microsoft\Windows\CurrentVersion\Applets\Regedit, LastKey, %MyComputer%\%f_Selected%
			Run regedit
		}
		else
			TrayTip, Error, Could not open " %f_Selected% " ., , 3
